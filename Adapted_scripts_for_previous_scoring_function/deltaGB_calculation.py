...
crisproff_path="path/Adapted_scripts_for_previous_scoring_function/crisproff_result"
...
for dataset in datasets:
    ...
    name=dataset.split("/")[-1].split(".")[0]
    if name == "E18_Cui":
        off_result=crisproff_path+"/E75_Rousset/params_out"        
    else:
        off_result=crisproff_path"/"+name+"/params_out"  
    print(name)
    ...
    df1.to_csv(crisproff_path+"/"+name+"_crisproff.csv",sep='\t',index=False)
